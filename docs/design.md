...
選択中の文は折り返して全文を読める（横スクロールだけで隠さない）。再分割のキャレット位置は維持する。文そのものは読み取り専用で編集しない。
