...
| 導入     | `#[derive(TS)]` と `cargo export-bindings` でファイル書き出し | 全コマンドに Specta、Builder の組み替え                            |
...
- 更新は `apps/app/src-tauri` で `cargo export-bindings`。`cargo test` では TypeScript を書かない。生成物はコミットする
- コマンドの登録の仕方は変えない
