...
- ブランチは `{type}/{issue番号}-{slug}`。type は `feat`（機能）/ `fix`（修正）/ `chore`（土台・掃除）。slug は英小文字とハイフンだけ。用語は CONTEXT.md のローマ字をケバブにする（例: `gakushu-gengo`）。1 issue に 1 ブランチ。名前は issue 本文の「ブランチ」行に従う
